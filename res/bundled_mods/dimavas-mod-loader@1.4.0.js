const METADATA = {
	website: "",
	author: "Dimava",
	name: "Dimava's Mod Loader (Bundled)",
	version: "1.4.0",
...
class Mod extends shapez.Mod {
	...
	modsState() {
		...
		q.orElm('head>style#qwe').innerHTML = `
			...
			[area="a"] {grid-area: a; whitespace: pre-line;}
			[area="b"] {grid-area: b; whitespace: pre-line;}
			[area="c"] {grid-area: c; whitespace: pre-line;}
			[area="d"] {grid-area: d; whitespace: pre-line;}
		`
		...
		srch.oninput = () => {
			...
			ta3.value = `// All mods on shapez.mods.io:\n`
				...
				+ dml.api.modList.map(e => e.name_id).sort()
					.filter(filter)
					.join('\n');
		}
		srch.oninput();
